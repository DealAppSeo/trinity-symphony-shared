...
`;
            ...
            if (!fastResult.passed) {
                if (gateEnabled) {
                    ...
                    const { postSubstanceGateEvent } = require('./substance-gate-client');
                    await postSubstanceGateEvent({
                        task,
                        fastResult,
                        agentName: this.name,
                        contentHash: require('./wrapper-patterns').computeContentHash(result.output),
                        shadow_mode: false
                    });
                ...
                // Pass path
                const { postSubstanceGateEvent } = require('./substance-gate-client');
                const gateResponse = await postSubstanceGateEvent({
                    task,
                    fastResult,
                    agentName: this.name,
                    contentHash: require('./wrapper-patterns').computeContentHash(result.output),
                ...
                if (queueEnabled && tier >= minTier && gateResponse?.gateEventId) {
                    await this.supabase.from('trinity_tasks').update({
