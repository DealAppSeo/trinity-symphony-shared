...
const RETRY_BACKOFF_MS = 250; // small backoff before the single 429 retry
...
async function recordEvent({ task, output, substanceGate, agentName }) {
  ...
  // Single POST attempt. Returns the same normalized shape recordEvent returns,
  // so the retry wrapper can decide to reuse or replace the result.
  const attempt = async () => {
  ...
  // First try. On a transient 429 (rate-limited — could not record the gate
  // event) retry exactly once after a small backoff before giving up. A gate
  // is a control surface: a recording failure must degrade loudly (caller-side),
  // never silently pass — so we give the record one more chance first.
  let res = await attempt();
  if (res.degraded && res.httpStatus === 429) {
