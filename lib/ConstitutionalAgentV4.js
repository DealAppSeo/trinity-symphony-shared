...
const GROQ_MODEL = 'openai/gpt-oss-20b'; // verified live 2026-09-10 20:31Z; llama-3.3-70b-versatile retired on free

/** Greppable success-path line. Deploy logs must show which model actually ran. */
function formatLlmSuccessLine(provider, model, task) {
    return `[LLM] provider=${provider} model=${model} task=${task == null ? '' : task}`;
}

const PROVIDERS = {
    groq: { baseUrl: 'https://api.groq.com/openai/v1/chat/completions', envKey: 'GROQ_API_KEY', model: GROQ_MODEL, priority: 1 },
    // cerebras PARKED — catalog was substituting gemma-4-31b → 404. Do not ship that id. Optional later: gpt-oss-120b after a live 200.
    cerebras: { baseUrl: 'https://api.cerebras.ai/v1/chat/completions', envKey: 'CEREBRAS_API_KEY', model: 'PARKED', priority: 1 },
    ...
    openrouter: { baseUrl: 'https://openrouter.ai/api/v1/chat/completions', envKey: 'OPENROUTER_API_KEY', model: process.env.OPENROUTER_MODEL || 'deepseek/deepseek-chat', priority: 1, isOpenRouter: true },
    ...
    nvidia: { baseUrl: 'https://integrate.api.nvidia.com/v1/chat/completions', envKey: 'NVIDIA_API_KEY', model: 'meta/llama-3.1-8b-instruct', priority: 2 },
...
class ConstitutionalAgentV4 {
    ...
    detectProviders() {
        return Object.keys(PROVIDERS).filter((k) => {
            if (k === 'cerebras') return false; // parked — model-gone, not a 429
            if (k === 'nvidia') return !!(process.env.NVIDIA_API_KEY || process.env.NVIDIA_NIM_API_KEY);
            return !!process.env[PROVIDERS[k].envKey];
...
`;
    ...
    async processPeerVerifyTask(task) {
        ...
        let queueEntries;
        try {
            // THIS WAS NOT AN EXCLUSIVE CLAIM. The predicate re-accepted 'in_review',
            // so a row another verifier was actively working could be claimed again —
            // and because the verdict is POSTed in a SEPARATE step after an LLM call,
            // a worker that died in between left the row `in_review` forever with no
            // way to tell a stale claim from a live one. 62,841 rows wedged exactly
            // that way, frozen since 2026-07-21 [MEASURED 2026-09-22].
            //
            // `claimed_at` (migration 2026_09_22_peer_verification_queue_lease_and_closure,
            // APPLIED) makes the two distinguishable, so the claim can now be strict:
            // take a row that is `pending`, or one whose lease has actually expired.
            // A live claim keeps its claimed_at fresh, so its window never opens.
            // A BLIND 2-of-3 PANEL HAS NO SINGLE OWNER, SO IT MUST NOT TAKE AN
            // EXCLUSIVE CLAIM. [MEASURED 2026-09-23] The strict predicate below was
            // added 2026-09-22 to stop rows wedging, and it does — but the panel path
            // starves on it, because peer-verification-reader.ts ALREADY set
            // verification_status='in_review' and stamped a FRESH claimed_at before
            // dispatching one task per verifier. For all three the predicate then
            // matches nothing (not `pending`, lease not expired), all three write
            // "Soft-skipped: … already processed by another verifier", the lease
            // expires 30m later, the reader reclaims and redispatches, and it repeats.
            // 26,652 rows are in that loop and no row has reached `verified` since
            // 2026-07-04.
            //
            // Duplicate ballots are already impossible WITHOUT this lock:
            // peer_verification_votes carries UNIQUE(source_response_id,
            // verifier_agent_id) and the insert in repid-engine's
            // routes/peer-verification.ts is ON CONFLICT DO NOTHING — one ballot per
            // verifier, idempotent on retry. The lock buys nothing here and costs
            // every vote.
            //
            // The single-verifier path keeps the strict claim unchanged: there the row
            // IS owned by one worker, and the 2026-09-22 anti-wedge fix is what makes a
            // dead claim distinguishable from a live one. Only the panel branch is new.
            // A 0-row result below therefore means something different per branch, and
            // the log line says which: for a panel it is a genuinely CLOSED queue row
            // (verified/disputed/timeout), never contention.
            const isPanelTask = task.metadata?.panel === 'blind_2of3';
            if (isPanelTask) {
                queueEntries = await pgQuery(
                    `SELECT id, claim_text, certainty_at_claim, source_agent_id, verification_status
                       FROM peer_verification_queue
                      WHERE id = $1
                        AND verification_status IN ('pending', 'in_review')`,
                    [queueId],
                    { retries: 1, label: 'peer-verify-panel-read' }
                );
                console.log(
                    `[${this.name}] Panel task ${task.id}: read queue ${queueId} without an exclusive claim ` +
                    `(blind 2-of-3; ${queueEntries?.length ? 'open' : 'closed'}).`
                );
            } else {
...
Provide your reasoning and analysis first, and end with the [VERDICT] line.`;
        ...
        try {
            ...
            // SECOND RECURSION SEED, AND THE ONE THAT PROVES THE CONTENT FENCE IS THE WRONG
            // FENCE. This read `Peer verification submitted verdict: timeout (...)`, and the
            // guard in repid-engine is `RE_RECURSIVE = /peer verification completed/i`.
            // "submitted" is not "completed", so this sibling line — in the same method as the
            // string the regex WAS written for — walked straight past it.
            //
            // Not hypothetical: 2 rows in peer_verification_queue carry exactly this text,
            // written 2026-07-06 [MEASURED 2026-09-22]. The regex never matched them. Only the
            // structural guard (metadata.peer_verification_queue_id) and low volume kept it
            // from compounding the way the 36,193 'completed' rows did.
            //
            // So this path gets the same treatment as the success path: a marker that is not a
            // work claim, with the verdict carried structurally where a machine reads it.
            await this.supabase.from('trinity_tasks').update({
                status: 'done',
                result: `[PEER_VERIFY] timeout verdict recorded in metadata.peer_verify (queue ${queueId})`,
                completed_at: new Date().toISOString(),
                metadata: {
                    ...(task.metadata || {}),
                    peer_verification_queue_id: queueId,
                    peer_verify: {
                        verdict: 'timeout',
                        verifier_response_id: post.verifierResponseId,
                        queue_id: queueId,
                        verifier_agent: this.name,
                        timeout_reason: llmErr.message
                    },
        ...
        //
        // THE `result` FIELD IS WHAT THE BRIDGE ENQUEUES AS A CLAIM. It used to read
        //   `Peer verification completed with verdict: ${verdict}. Response ID: ...`
        // and 36,193 rows of peer_verification_queue are that sentence, re-enqueued as
        // claims to be verified — the queue verifying its own output [MEASURED
        // 2026-09-22; those rows are now closed `stale`/recursive_meta_verification].
        //
        // Two fences already stop it, and BOTH are weaker than they look:
        //   - a CONTENT regex, `RE_RECURSIVE = /peer verification completed/i`. One
        //     reword of this template ("Verification finished with verdict:") and it
        //     silently stops matching. A guard keyed to prose we control here is a
        //     guard that a routine copy-edit disables.
        //   - a STRUCTURAL check, `isPeerVerificationTask()`, which is the real one:
        //     task_type==='peer_verify' OR metadata.peer_verification_queue_id present.
        //
        // So the durable fix is to stop emitting a claim-shaped sentence at all, and to
        // make the structural markers EXPLICIT rather than incidental. The verdict is
        // not lost — it moves into metadata as structured data, which is where a machine
        // should have been reading it from in the first place. `result` keeps a marker
        // that is not a work claim and cannot be mistaken for one by a reader or a model.
        await this.supabase.from('trinity_tasks').update({
            status: 'done',
            result: `[PEER_VERIFY] verdict recorded in metadata.peer_verify (queue ${queueId})`,
            completed_at: new Date().toISOString(),
            metadata: {
                ...(task.metadata || {}),
                // Carried EXPLICITLY, not left to the spread above. isPeerVerificationTask()
                // keys on this, so a future refactor of task.metadata must not be able to
                // drop it silently — that single field is what keeps the loop bounded.
                peer_verification_queue_id: queueId,
                peer_verify: {
                    verdict,
                    verifier_response_id: post.verifierResponseId,
    ...
    async _postPeerVerdict(queueId, verdict) {
        ...
        // NO FALLBACK. This used to default to the literal 'test-key-123', which meant a
        // service missing REPID_API_KEY silently POSTed every verdict with an invalid
        // credential: the engine rejects it, the row stays `in_review`, and the failure
        // is indistinguishable from a dead verifier. That is a second, independent way
        // to manufacture the exact wedge above — and it fails SILENTLY, which is worse
        // than failing loudly. Refuse instead, and say which variable is missing.
        const apiKey = process.env.REPID_API_KEY;
        if (!apiKey) {
            throw new Error(
                'REPID_API_KEY is not set on this service. Refusing to POST a peer-verification ' +
                'verdict with no credential: the engine would reject it and the queue row would ' +
                'strand in in_review, looking like a dead verifier rather than a config error.'
            );
        }
    ...
    async callLLM(prompt, options = {}) {
        ...
        const freeTier = require('./free-tier-gate');
        try {
            const { canUseProxy, callEngineComplete } = require('./engine-llm-proxy');
            if (canUseProxy() && freeTier.allowPaid()) {
        ...
        const ordered = freeTier.orderProviders(
            this.availableProviders
                .filter((k) => PROVIDERS[k])
                .map((k) => ({
                    provider: k,
                    model: k === 'openrouter'
                        ? (process.env.OPENROUTER_MODEL || PROVIDERS[k].model)
                        : PROVIDERS[k].model,
                })),
            { allowPaid: freeTier.allowPaid() },
        );
        if (ordered.length === 0) {
            if (freeTier.allFreeExhausted()) throw new Error('FREE_EXHAUSTED');
            throw new Error('All LLMs failed');
        }
        for (const slot of ordered) {
            const providerKey = slot.provider;
            ...
            try {
                ...
                console.log(formatLlmSuccessLine(
                    canonicalizeProvider(providerKey) || providerKey,
                    slot.model || provider.model,
                    this.currentTaskId || '',
                ));
                return { ...res, provider: canonicalizeProvider(providerKey) };
            } catch (e) {
                console.warn(`${providerKey} failed`);
                if (freeTier.isQuotaExhaustedError(e && e.message)) freeTier.markExhausted(providerKey);
            }
        }
        if (freeTier.allFreeExhausted()) throw new Error('FREE_EXHAUSTED');
        throw new Error('All LLMs failed');
    ...
    async callProvider(provider, prompt) {
        const apiKey = process.env[provider.envKey] || (provider.envKey === 'NVIDIA_API_KEY' ? process.env.NVIDIA_NIM_API_KEY : undefined);
...
module.exports.GROQ_MODEL = GROQ_MODEL;
module.exports.PROVIDERS = PROVIDERS;
module.exports.formatLlmSuccessLine = formatLlmSuccessLine;
