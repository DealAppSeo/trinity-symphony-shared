...
`;
    ...
    async callMcpTool(mcpName, toolName, args = {}) {
        const url = process.env.MCP_A2A_GATEWAY_URL || 'http://localhost:8010';
        try {
            console.log(`[MCP] 🔌 ${this.name} calling ${mcpName}/${toolName}...`);
            ...
            if (!response.ok) throw new Error(result.error || 'Failed to call MCP Gateway');
            return result;
        } catch (error) {
    ...
    async callProvider(provider, prompt) {
        ...
        const tools = [
            ...
            },
            {
                ...
                function: {
                    name: 'call_mcp_tool',
                    description: 'Calls an external Model Context Protocol (MCP) tool via the A2A Gateway.',
                    parameters: {
                        type: 'object',
                        properties: {
                            mcp_name: { type: 'string', description: 'Name of the MCP server (e.g. web_search, github)' },
                            tool_name: { type: 'string', description: 'Name of the specific tool to call on that server' },
                            args: { type: 'object', description: 'Arguments payload for the tool as a JSON object' }
                        },
                        required: ['mcp_name', 'tool_name', 'args']
                    }
                }
            }
        ];
            ...
            if (toolCalls && toolCalls.length > 0) {
                for (const toolCall of toolCalls) {
                    ...
                    if (fnName === 'save_artifact') {
                        ...
                        const result = await this.callMcpTool(args.mcp_name, args.tool_name, args.args);
    ...
    async saveArtifact(taskId, content, type = 'markdown', title = null) {
        ...
        try {
            ...
            const { data, error } = await this.supabase.from('trinity_artifacts').insert({
                ...
                agent: this.name,
                filename: `artifact-${safeTaskId}.md`,
