            ...
            try {
                ...
                if (!task) {
                    ...
                    try {
                        ...
                        const sc = await ServiceContractClient.processOne(this.name);
                        if (sc && sc.processed) {
                    ...
                    // SPRINT 3: Evergreen Rotation Pattern
                    try {
                        const evergreenTask = await this.pollEvergreenTasks();
                        if (evergreenTask) {
                            console.log(`[${this.name}] 🌱 Spawning Trinity Task from Evergreen Template: ${evergreenTask.title}`);
                            const { data, error } = await this.supabase.from('trinity_tasks').insert({
                                title: `[EVERGREEN] ${evergreenTask.title}`,
                                description: `Autonomous Evergreen Task spawned from table: ${evergreenTask.source_table}\n\nTask Type: ${evergreenTask.task_type}\n\nExecute this evergreen maintenance task according to standard protocol.`,
                                success_criteria: `Produce a valid artifact summarizing the results of this evergreen task (${evergreenTask.title}).`,
                                priority: evergreenTask.priority || 5,
                                status: 'pending',
                                claimed_by: null,
                                metadata: {
                                    is_evergreen: true,
                                    source_table: evergreenTask.source_table,
                                    source_id: evergreenTask.id
                                }
                            }).select('id').single();
                            
                            if (error) {
                                console.error(`[${this.name}] failed to spawn evergreen task:`, error.message);
                            } else {
                                continue;
                            }
                        }
                    } catch (e) {
                        console.error(`[${this.name}] evergreen poll error:`, e?.message);
                    }
...
`;
    ...
    // --- SPRINT 3: EVERGREEN TASK ROTATION ---
    async pollEvergreenTasks() {
        try {
            // Randomly select one of the three tables to avoid starvation if one is full
            const tables = ['hal_evergreen_tasks', 'trinity_evergreen_tasks', 'evergreen_tasks'];
            // Shuffle
            ...
            for (const table of tables) {
                ...
                if (table === 'hal_evergreen_tasks') {
                    query = query.eq('status', 'available').order('priority', { ascending: false });
                } else if (table === 'trinity_evergreen_tasks') {
                    query = query.eq('active', true).order('priority', { ascending: false });
                } else if (table === 'evergreen_tasks') {
                    query = query.eq('enabled', true).order('priority', { ascending: false });
                }
                
                const { data, error } = await query.limit(1);
                if (error) {
                    console.error(`[${this.name}] Error querying ${table}:`, error.message);
                    continue;
                }
                ...
                if (data && data.length > 0) {
                    ...
                    return {
                        ...
                        task_type: task.task_type || task.frequency || 'maintenance',
                        priority: task.priority
                    };
                }
            }
        } catch (e) {
            console.error(`[${this.name}] Error in pollEvergreenTasks:`, e.message);
        }
        return null;
    }
