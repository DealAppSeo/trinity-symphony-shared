...
`;
    ...
    async processPeerVerifyTask(task) {
        ...
        // 1. Fetch the peer_verification_queue entry
        const { data: queueEntry, error: queueErr } = await this.supabase
        ...
        if (queueErr || !queueEntry) {
            throw new Error(`Failed to fetch queue entry ${queueId}: ${queueErr?.message || 'Not found'}`);
        }
