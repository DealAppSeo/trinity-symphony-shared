...
`;
        ...
        if (!substanceGate.ok) {
            if (gateEnabled) {
                ...
                // gate FAILED but the event could NOT be recorded (429/timeout/unreachable):
                // a gate degrades LOUDLY and holds — never a silent PASS. Fail-closed.
                await this._holdTaskGateDegraded(task, result, substanceGate, gateRecord);
                return;
            }
        }
...
Provide your reasoning and analysis first, and end with the [VERDICT] line.`;
    ...
    async _postSubstanceGateEvent(task, output, substanceGate) {
        try {
            ...
            if (res.degraded) {
                // A degraded record only PASSES when the gate itself passed; a FAILED
                // gate that couldn't be recorded is held fail-closed by the caller.
                const gatePassed = !!(substanceGate && substanceGate.ok);
                console.warn(`[SUBSTANCE_GATE] ⚠️ event NOT recorded (${res.degradeReason}) task ${task.id} — ${gatePassed ? 'gate passed, proceeding' : 'gate FAILED → will HOLD (fail-closed)'}`);
                await this.log('substance_gate_degraded', `Gate event degraded: ${res.degradeReason} (gatePassed=${gatePassed})`, { taskId: task.id, degradeReason: res.degradeReason, gatePassed, claimed_by: this.name });
    ...
    /**
     * A FAILING substance gate whose event could NOT be recorded (429 / timeout /
     * unreachable) must NOT silently pass — a gate degrades LOUDLY and holds.
     * Fail-closed: mark the task held/unverified, record the degradation in
     * metadata, and log clearly. Non-crashing.
     */
    async _holdTaskGateDegraded(task, result, substanceGate, gateRecord) {
        const degradeReason = (gateRecord && gateRecord.degradeReason) || 'gate_unrecorded';
        console.warn(`[SUBSTANCE_GATE] ⛔ HOLD task ${task.id}: substance gate FAILED (${substanceGate.reason}) and event could not be recorded (${degradeReason}) — degrading loudly, NOT passing`);
        await this.log('substance_gate_held_unverified', `Task ${task.id} held: gate failed + record degraded (${degradeReason})`, {
            taskId: task.id, degradeReason, gateReason: substanceGate.reason, claimed_by: this.name
        });
        try {
            await this.supabase.from('trinity_tasks').update({
                ...
                metadata: {
                    ...
                    provider_used: result.provider,
                    provider: result.provider,
                    substance_gate_degraded: true,
                    substance_gate_degrade_reason: degradeReason,
                    substance_gate_failure: (gateRecord && gateRecord.fastResult) ? gateRecord.fastResult.failures : [substanceGate.reason],
                    substance_gate_reasoning: substanceGate.reason
                }
            }).eq('id', task.id);
        } catch (e) {
            console.error(`[SUBSTANCE_GATE] ❌ failed to persist held_unverified for task ${task.id}: ${e.message}`);
        }
        this.currentTaskId = null;
    }
    ...
    /**
     * Record per-(agent, provider) performance into the ANFIS provider-perf store
     * so the router can weight providers by speed/success. Writes to the existing
     * `provider_performance` table (unique key (agent, model)); `model` holds the
     * provider key. Read-modify-write on the running aggregate. Non-fatal: any DB
     * failure is swallowed so provider tracking never crashes the agent.
     */
    async trackProviderPerformance(providerKey, success, latencyMs) {
        if (!providerKey || !this.supabase) return;
        const latency = Number.isFinite(latencyMs) ? latencyMs : 0;
        try {
            const { data: existing } = await this.supabase
                .from('provider_performance')
                .select('total_calls, successes, avg_latency_ms')
                .eq('agent', this.name)
                .eq('model', providerKey)
                .maybeSingle();

            if (existing) {
                const totalCalls = (existing.total_calls || 0) + 1;
                const successes = (existing.successes || 0) + (success ? 1 : 0);
                const avgLatency = ((Number(existing.avg_latency_ms) || 0) * (existing.total_calls || 0) + latency) / totalCalls;
                await this.supabase
                    .from('provider_performance')
                    .update({
                        total_calls: totalCalls,
                        successes,
                        success_rate: successes / totalCalls,
            ...
            // Non-fatal: provider-perf tracking must never crash the agent.
            console.warn(`[ANFIS] provider-perf write skipped (${providerKey}): ${e.message}`);
        }
    }
