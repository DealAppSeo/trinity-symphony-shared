...
const ReceiptValidator = require('./receipt-validation/ReceiptValidator');
...
`.trim();
    ...
    async processTask(task) {
        ...
        if (task.task_type === 'receipt_validation') {
            await this.processReceiptValidationTask(task);
            return;
        }
...
`;
    ...
    async processReceiptValidationTask(task) {
        console.log(`[RECEIPT] ${this.name} validating receipt ${task.metadata?.receiptId}`);
        try {
            // Fetch receipt JSON. Depending on how it's stored, we'll try to fetch it if metadata has URI,
            // or we expect CC1's indexer to have saved the payload in hyperdag_receipts.
            let receiptJson = task.metadata?.payload;
            if (!receiptJson) {
                const { data } = await this.supabase
                    .from('hyperdag_receipts')
                    .select('payload')
                    .eq('receipt_id', task.metadata?.receiptId)
                    .single();
                receiptJson = data?.payload;
            }

            if (!receiptJson && task.metadata?.receiptUri && task.metadata.receiptUri.startsWith('http')) {
                const res = await fetch(task.metadata.receiptUri);
                receiptJson = await res.json();
            }

            if (!receiptJson) {
                throw new Error("Could not fetch receipt JSON for validation");
            }

            const context = {
                agentName: this.name.replace('trinity-', '').toUpperCase(),
                agentVersion: this.version,
                halOutput: receiptJson.hal
            };

            const attestation = await this.receiptValidator.validate(receiptJson, context);

            // Save to trinity_receipt_validators
            await this.supabase.from('trinity_receipt_validators').upsert({
                receipt_id: task.metadata.receiptId,
                agent_name: attestation.agentName,
                verdict: attestation.verdict,
                confidence: attestation.confidence,
                reasoning_hash: attestation.reasoningHash,
                signature: attestation.signature,
                attested_at: attestation.timestamp
            }, { onConflict: 'receipt_id,agent_name' });

            // If ORCH, try BFT aggregation
            if (this.name === 'trinity-orch' || this.name === 'ORCH') {
                const { data: allAttestations } = await this.supabase
                    .from('trinity_receipt_validators')
                    .select('*')
                    .eq('receipt_id', task.metadata.receiptId);
                
                if (allAttestations) {
                    const bftResult = await this.receiptValidator.aggregateAttestations(allAttestations);
                    console.log(`[RECEIPT BFT] ORCH evaluated BFT for ${task.metadata.receiptId}: ${bftResult.verdict}`);
                    // The indexer also does this, but doing it here aligns with ORCH agent responsibility
                }
            }

            await this.supabase.from('trinity_tasks').update({
                status: 'done',
                result: `Validated with verdict: ${attestation.verdict}`,
                completed_at: new Date().toISOString()
            }).eq('id', task.id);

        } catch (e) {
            console.error(`[RECEIPT] Validation failed for task ${task.id}: ${e.message}`);
            await this.supabase.from('trinity_tasks').update({
                status: 'failed',
