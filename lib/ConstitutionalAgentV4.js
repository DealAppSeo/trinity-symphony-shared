...
const { provenance, inheritProvenance } = require('./provenance');
...
`;
    ...
    async saveArtifact(taskId, content, type = 'markdown', title = null) {
        ...
        if (taskId) {
            const { data } = await this.supabase.from('trinity_tasks').select('metadata').eq('id', taskId).single();
            if (data && data.metadata) {
                payload.metadata = inheritProvenance(data.metadata, 'trinity_artifacts');
            } else {
                payload.metadata = inheritProvenance({}, 'trinity_artifacts');
            }
        } else {
            payload.metadata = inheritProvenance({}, 'trinity_artifacts');
        }
    ...
    async log(action, message, metadata = {}) {
        try {
            ...
            const mergedMetadata = { ...metadata, ...prov, version: this.version, squad: this.wisdom.squad };
