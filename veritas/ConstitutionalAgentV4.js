/**
 * TRINITY CONSTITUTIONAL AGENT V4 (LOCAL COPY) - VERITAS
 */
...
class ConstitutionalAgentV4 {
    ...
    determineGroup(name) {
        const groups = { 'COMMUNICATION': ['GABRIEL', 'RAZIEL', 'CASSIEL'], 'ORCHESTRATION': ['TORCH', 'ZADKIEL', 'HANIEL'], 'CREATION': ['CHESED', 'AURIEL', 'URIEL'], 'EXECUTION': ['SOPHIA', 'MICHAEL', 'RAPHAEL'], 'CONDUCTOR': ['API', 'MCP', 'ORCHESTRATOR', 'APM', 'GCM', 'HDM', 'MEL', 'VERITAS', 'W3C'] };
        for (const [group, members] of Object.entries(groups)) { if (members.includes(name)) return group; }
        return 'UNKNOWN';
    }
    async start() {
        console.log('========================================');
    ...
    async runLoop() {
        ...
        while (true) {
            try {
                ...
                if (task) { console.log(`[${this.name}] 📋 Processing: ${task.title}`); await this.processRouter(task); }
    ...
    canHandleLocally(task) { const localTypes = ['self-healing', 'system', 'heartbeat', 'wake', 'meta']; const localTitles = ['[HEALING]', '[HEARTBEAT]', '[SYSTEM]']; return localTypes.includes(task.task_type) || (task.title && localTitles.some(t => task.title.includes(t))); }
